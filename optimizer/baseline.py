...
class RedTeamingMethod:
    ...
    # default_dependencies = [transformers,  ray, fastchat]
    """
...
class SingleBehaviorRedTeamingMethod(RedTeamingMethod):
    ...
    def generate_test_cases(self, behaviors, target_sentence=None, verbose=False):
        """
        ...
        # if self.method_name in ['GCG', 'AutoDAN']:
        #     model, tokenizer = self.model, self.tokenizer
        # else:
        #     model, tokenizer = None, None
        model, tokenizer = self.model, self.tokenizer
        # model, tokenizer = load_model_and_tokenizer(**self.model_kwargs)
        ...
        for behavior_dict in tqdm(behaviors, total=len(behaviors)):
            ...
            for j in range(num_batches):
                ...
                batch_test_cases, batch_logs = self.generate_test_cases_single_behavior(
                    model=model, tokenizer=tokenizer, behavior_dict=behavior_dict, num_generate=current_batch_size, 
                    target_sentence=target_sentence, verbose=verbose)
            ...
            # behavior_id = behavior_dict['behavior_id']
        ...
        print(f'Number of generated test cases: {len(test_cases)}')
    ...
    def get_output_file_path(save_dir, behavior_id, file_type, run_id=None):
        ...
        run_id= ""
        return os.path.join(save_dir, 'test_cases_individual_behaviors', str(behavior_id), f"{file_type}{run_id}.json")
