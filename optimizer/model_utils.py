...
def get_template_orig(model_name_or_path, chat_template=None, fschat_template=None, system_message=None, return_fschat_conv=False, **kwargs):
    ...
    elif "vicuna" in chat_template:
        TEMPLATE = VICUNA_PROMPT
    elif chat_template == "oasst":
        TEMPLATE = OASST_PROMPT
    elif chat_template == "oasst_v1_1":
        TEMPLATE = OASST_PROMPT_v1_1
    elif chat_template == "llama-2" or "llama" in chat_template or 'struq' in chat_template or 'alpaca' in chat_template:
...
def get_template(tokenizer=None, model_name_or_path=None, chat_template=None, system_message=None, return_fschat_conv=False, **kwargs):
    if chat_template == "wizard":
        ...
        TEMPLATE = OASST_PROMPT_v1_1
    elif chat_template == "llama-2" or "llama" in chat_template.lower() or 'struq' in chat_template.lower() or 'alpaca' in chat_template.lower():
        TEMPLATE = LLAMA2_CHAT_PROMPT
    elif chat_template == "falcon_instruct": #falcon 7b / 40b instruct
        TEMPLATE = FALCON_INSTRUCT_PROMPT
    elif chat_template == "falcon_chat": #falcon 180B_chat
    ...
    print("Found Instruction template for", chat_template)
...
def _get_fschat_conv(model_name_or_path=None, fschat_template=None, system_message=None, **kwargs):
    ...
    if template_name is None:
        ...
        ## Set default template name to llama-2
        if template.dict()['template_name'] == 'one_shot' or template is None:
            print(f"Set chat template to llama-2")
...
def load_vllm_model(
    ...
    **kwargs
    ...
    model.llm_engine.tokenizer.tokenizer.max_length = 4096
    return model
