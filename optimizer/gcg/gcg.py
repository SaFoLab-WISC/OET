...
from ..model_utils import get_template_orig as get_template
...
class GCG(SingleBehaviorRedTeamingMethod):
    ...
    def generate_test_cases_single_behavior(
            self, behavior_dict, verbose=False, target_sentence=None, **kwargs):
        """
        ...
        # cache_input_ids = []
        # cache_input_ids += tokenizer([before_adv, after_adv, target], padding=False, add_special_tokens=False)['input_ids']
        # cache_input_ids = [torch.tensor(input_ids, device=device).unsqueeze(0) for input_ids in cache_input_ids]
        # before_adv_ids, after_adv_ids, target_ids = cache_input_ids
        # before_adv_embeds, after_adv_embeds,  target_embeds = [embed_layer(input_ids) for input_ids in cache_input_ids]
        ...
        if self.use_prefix_cache:
            ...
            # input_embeds = torch.cat([before_adv_embeds], dim=1)
            with torch.no_grad():
        ...
        for i in range(num_steps):
            ...
            if self.use_prefix_cache:
                input_embeds = torch.cat([optim_embeds, after_adv_embeds, after_embeds, 
                                          target_embeds
                                        ],dim=1)
                # input_embeds = torch.cat([optim_embeds, after_adv_embeds, target_embeds], dim=1)
                outputs = model(inputs_embeds=input_embeds, past_key_values=self.prefix_cache)
            else:
                input_embeds = torch.cat([before_embeds, before_adv_embeds, optim_embeds, after_adv_embeds, after_embeds, 
                                          target_embeds
                                          ], dim=1)
                # input_embeds = torch.cat([befor e_adv_embeds, optim_embeds, after_adv_embeds, target_embeds], dim=1)
                outputs = model(inputs_embeds=input_embeds)
            ...
            #         top_k_values, top_k_indices = torch.topk(logits, top_k)  # Keep only top-k values
            #         probs = torch.nn.functional.softmax(top_k_values, dim=-1)  # Convert to probabilities
            #         sampled_token_id = torch.multinomial(probs, num_samples=1)  # Sample from distribution
            #         return top_k_indices[0, sampled_token_id].item() 


            #     # embedd_layer = model.get_input_embeddings()
            #     # embedding = embedd_layer(input_ids)
            #     # output = model(inputs_embeds=embedding)

            #     max_new_tokens = 20
            #     input_ids = 
            #     # input_ids = [torch.tensor(input_ids, device=device).unsqueeze(0) for input_ids in cache_input_ids[:-1]]
            #     # generated_tokens = input_ids.tolist()[0]  # Start with initial tokens
            #     # generated_tokens = [before_ids, before_adv_ids, after_adv_ids, after_ids]
            #     generated_tokens = []
            ...
            #             else:
            #                 inputs_embeds = embed_layer(torch.tensor([generated_tokens]).to('cuda'))
            #             outputs = model(inputs_embeds=inputs_embeds)
            #             logits = outputs.logits[:, -1, :]

            #         # Sample next token
            #         sampled_token_id = sample_from_logits(logits, temperature=0.2, top_k=50)
            #         generated_tokens.append(sampled_token_id)
            ...
            adv_string = tokenizer.decode(optim_ids[0]).strip()
            
            # test_case_ids = torch.cat([context_ids, optim_ids], dim=1)
            # test_case = tokenizer.decode(test_case_ids[0])
            current_loss = loss.min().item()
            all_losses.append(current_loss)
            suffixs.append(adv_string)
