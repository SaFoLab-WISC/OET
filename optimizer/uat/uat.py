...
class UAT(SingleBehaviorRedTeamingMethod):
    ...
    def generate_test_cases_single_behavior(self, behavior_dict, target_sentence=None, verbose=False, **kwargs):
        ...
        before_adv_ids = tokenizer(before_adv, return_tensors="pt", add_special_tokens=False).to(device)['input_ids']
        before_adv_embeds = embed_layer(before_adv_ids)
        ...
        after_adv_ids = tokenizer(after_adv, return_tensors="pt", add_special_tokens=False).to(device)['input_ids']
        after_adv_embeds = embed_layer(after_adv_ids)
        # context_ids = tokenizer(context_str, return_tensors="pt", add_special_tokens=False).to(device)['input_ids']
        ...
        if self.use_prefix_cache:
            ...
            init_input_embeds = torch.cat([before_embeds, before_adv_embeds], dim=1)
            with torch.no_grad():
        ...
        # all_test_cases = []
        suffixs = []
        pbar = tqdm(range(num_steps), desc=f"UAT")
        for i in range(num_steps):
            ...
            for token_to_flip in range(0, trigger_token_length):
                ...
                if self.use_prefix_cache:
                    input_embeds_candidates = torch.cat([candidates_embeds,
                                                         after_adv_embeds.repeat(len(candidates), 1, 1),
                    ...
                    input_embeds_candidates = torch.cat([before_embeds.repeat(len(candidates), 1, 1),
                                                    before_adv_embeds.repeat(len(candidates), 1, 1),
                                                    candidates_embeds,
                                                    after_adv_embeds.repeat(len(candidates), 1, 1),
                                                    after_embeds.repeat(len(candidates), 1, 1),
                                                    target_embeds.repeat(len(candidates), 1, 1)], dim=1)
                ...
                if self.use_prefix_cache:
                    ...
                    # input_embeds = torch.cat([before_embeds, context_embeds, embed_layer(trigger_tokens.unsqueeze(0)), behavior_embeds, after_embeds, target_embeds], dim=1)
                    input_embeds = torch.cat([
                        before_embeds, 
                        before_adv_embeds,
                        embed_layer(trigger_tokens.unsqueeze(0)), 
            ...
            # test_case_ids = torch.cat([context_ids, trigger_tokens.unsqueeze(0), behavior_ids], dim=1)
            # test_case = tokenizer.decode(test_case_ids[0])
            suffixs.append(tokenizer.decode(trigger_tokens.unsqueeze(0)[0]))

            all_losses.append(curr_best_loss.item())
            pbar.update(1)
            pbar.set_description(f'Training Loss: {curr_best_loss.item()}')
        ...
        pbar.close()
        logs = {'final_loss': best_loss.item(), 
