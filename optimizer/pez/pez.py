...
class PEZ(SingleBehaviorRedTeamingMethod):
    ...
    def generate_test_cases_single_behavior(self, behavior_dict, target_sentence=None, verbose=False):
        ...
        for i in range(num_steps):
            ...
            nn_indices[nn_indices >= vocab_size] = 0

            suffix = tokenizer.batch_decode(nn_indices, skip_special_tokens=True)
            # nn_indices = torch.cat([before_adv_embeds.repeat(num_generate, 1), nn_indices, after_adv_embeds.repeat(num_generate, 1)], dim=1)
            ...
            all_suffix.append(suffix[best_idx])
